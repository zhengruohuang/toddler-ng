...
static shiftbuf_t or1k_ns16550_shiftbuf = { .value = 0 };
...
struct or1k_ns16550_driver {
    unsigned long seq;
    unsigned long mmio_base;
};

static struct or1k_ns16550_driver or1k_ns16550;
 ...
 * Read and write
 */
static inline int or1k_ns16550_received()
{
    u8 stat = __mmio_read8(or1k_ns16550.mmio_base + UART_LINE_STAT_REG) & 0x1;
    return stat ? 1 : 0;
}

static inline int or1k_ns16550_read_one()
{
    return __mmio_read8(or1k_ns16550.mmio_base + UART_DATA_REG);
}
 ...
 * Interrupt
 */
static sema_t or1k_ns16550_sema = SEMA_INITIALIZER(sizeof(unsigned long) - 1);
...
static void or1k_ns16550_int_handler(unsigned long param)
{
    ...
    // Read all
    int rc = 0;
    while (or1k_ns16550_received()) {
        int c = or1k_ns16550_read_one();
        switch (c) {
        case 0x7f:
            c = '\b';
            break;
        ...
        shiftbuf_write_one(&or1k_ns16550_shiftbuf, c);
        rc++;
    }

    // EOI
    syscall_int_eoi(or1k_ns16550.seq);

    // Enable interrupts
    __mmio_write8(or1k_ns16550.mmio_base + UART_INT_ENABLE_REG, 0x1);

    // Wake up readers
    if (rc) {
        sema_post_count(&or1k_ns16550_sema, rc);
    }
 ...
 * Driver ops
 */
static int dev_or1k_ns16550_read(devid_t id, void *buf, size_t count, size_t offset,
                            struct fs_file_op_result *result)
{
    ...
    do {
        sema_wait(&or1k_ns16550_sema);

        while (count) {
            unsigned char byte = 'a';
            int r = shiftbuf_read_one(&or1k_ns16550_shiftbuf, &byte);
            if (!r) {
                break;
            }
...
static int dev_or1k_ns16550_write(devid_t id, void *buf, size_t count, size_t offset,
                             struct fs_file_op_result *result)
{
    if (buf && count) {
        unsigned char *array = buf;
        for (size_t i = 0; i < count; i++) {
            or1k_ns16550_write_one(array[i]);
        }
    }
 ...
 * Init
 */
static const struct drv_ops dev_or1k_ns16550_ops = {
    .read = dev_or1k_ns16550_read,
    .write = dev_or1k_ns16550_write,
};

static inline void start_or1k_ns16550()
{
